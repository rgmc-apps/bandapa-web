...
export default function InviteClient({
    ...
    return (
      <Shell>
        <div className="text-center">
          ...
          <h1 className="font-headline font-bold text-white text-2xl mb-3">
            Invalid invite link
          </h1>
          <p className="text-white/40 text-sm leading-relaxed mb-8 max-w-[28ch] mx-auto">
            This invite code doesn&apos;t match any band. It may have been
            changed or is no longer active.
          </p>
          <a
            href="/"
            className="inline-flex items-center gap-2 text-sm font-mono text-chlorophyll hover:text-chlorophyll/80 transition-colors"
          >
    ...
    return (
      <Shell>
        <div className="text-center mb-5">
          <div className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-chlorophyll/10 border border-chlorophyll/20 rounded-full font-mono text-[11px] text-chlorophyll mb-4">
            <span
  ...
  // ── Logged in — can accept / decline ─────────────────────────────────
  if (userId) {
    return (
      <Shell>
        <div className="text-center mb-5">
          <p className="font-mono text-[11px] text-white/35 uppercase tracking-[0.15em] mb-2">
            Band invitation
          </p>
          <h1 className="font-headline font-bold text-white text-2xl leading-tight">
  ...
  return (
    <Shell>
      ...
      <div className="mt-5 space-y-2.5">
        ...
        <a
          href={`/register?next=${encodeURIComponent(inviteUrl)}`}
          className="block w-full py-3 border border-white/10 text-center text-white/45 hover:text-white/65 hover:border-white/20 rounded-xl text-sm font-mono transition-all active:scale-[0.97]"
        >
          Create an account
        </a>
      </div>
  ...
  return (
    <div className="bg-white/[0.04] border border-white/[0.07] rounded-xl p-4 flex items-center gap-4">
      ...
      <div className="flex-1 min-w-0">
        <p className="font-headline font-bold text-white text-lg leading-tight truncate">
          {band.name}
        </p>
        {genres.length > 0 ? (
          <div className="flex flex-wrap gap-1 mt-1.5">
            {genres.slice(0, 3).map((g) => (
              <span
                key={g}
                className="px-2 py-0.5 rounded-full bg-white/[0.06] text-white/35 text-[10px] font-mono border border-white/[0.06]"
              >
                {g}
              </span>
            ))}
